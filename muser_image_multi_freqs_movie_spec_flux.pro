;.r muser_image_multi_freqs_movie_spec_flux.pro
...
charsz=0.4*7 & charth=1.2*3 & th_ind =1
...
outfile = '/Users/xychen/Desktop/muser_image_L_center_spec_flux.mp4';movie/;;selfcal;satellite
...
;    SET_FONT='Helvetica',/ENCAPSULATED,scale_factor=2,/color
  ...
  xyouts,pos1[0]+0.40,pos1[1]+0.01, textoidl('SDO/AIA-')+strmid(string(indexa1.WAVELNTH),9,4)+' '+ $
...
;      ;dmin=5 & dmax=15 ;;for ch=0
      ...
      ;;============guassian fit
      index1=where(imgm ge 0.8*max(imgm));10.0 for flare;;3 for quiet ;;1 for left-right
...
;      
      ...
      plot_map,mapm,position = pos1,/notitle, $
        ...
        thick=charth*th_ind*1.2,charthick=charth,charsize=charsz,LEVELS=[5e6],/cont,color=250-(8*ch+ooo*127),/over
      ...
      nafik: 
      xyouts,pos1[2]-0.1,pos1[1]+0.02*(ch+2.5)*0.62+0.32*ooo*0.62, freqchstr+' MHz' ,$
...
;        /normal,color=8*ch+ooo*127,align=0,charsize=charsz,charthick=charth
  ...
  xyouts,pos1[2]-0.1-0.02,pos1[3]-0.053, 'MUSER-I-'+strmid(data_namet,8,1) ,$;+strmid(data_namet,8,1);(L-R)
  ...
  spectro_plot,sp,time,freqs,position=pos3,xstyle=1+4,ystyle=1+4 $
    ...
    ,ytitle='Frequency [MHz]',title='Spectrum-MUSER',xtitle=' ' $
    ,thick=charth*th_ind*1.2,charthick=charth,charsize=charsz,/noerase,color=-1
  axis,yaxis=0,yrange=[0.4,2],ystyle=1,charthick=charth,charsize=charsz,ytitle=textoidl('Frequency [GHz]'),color=255,ythick=charth*th_ind*1.2
  axis,yaxis=1,yrange=[0.4,2],charthick=charth,charsize=charsz,color=255,ythick=charth*th_ind*1.2,ytickname=replicate(' ',8)
  ...
  utplot,time,sp[*,48+4],position=pos3,xstyle=1,ystyle=1+4, $
    ...
    thick=charth*th_ind*1.2,charthick=charth,charsize=charsz,yrange=[0.8,2.2]
  ...
  ;axis,xaxis=1,charthick=charth,charsize=charsz,xtitle=' ',color=255,xthick=charth*th_ind*1.2,xtickname=replicate(' ',8)
  outplot,[anytim(indexa1.date_OBS),anytim(indexa1.date_OBS)],[0.8,2.2],thick=charth*th_ind*1.2,color=255;,linestyle=1
  
  xyouts,pos3[0]+0.02,pos3[3]-0.02, 'Flux-MUSER-(1.725 GHz)' ,$;+strmid(data_namet,8,1);(L-R)
    /normal,color=255,align=0,charsize=charsz/1.1,charthick=charth
;  xyouts,pos3[0]+0.02,pos3[3]-0.04, 'MUSER-Flux (1.725 GHz)',$;+strmid(data_namet,8,1);(L-R)
;    /normal,color=255,align=0,charsize=charsz/1.1,charthick=charth
