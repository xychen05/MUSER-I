;.r clean_sl_region_keep_table.pro
...
;;for it data
if sys eq 0 then filename=file_search('.\sav\'+data_namet+'\it_img_beam'+nm_ind+ktkt[kk]+'\*.sav',count=count) & $
  image_sav='.\imagetest\'+data_namet+'\it_clean_img'+nm_ind+ktkt[kk]+'\' & data_sav='.\sav\'+data_namet+'\it_clean_img'+nm_ind+ktkt[kk]+'\'
if sys eq 1 then filename=file_search('./sav/'+data_namet+'/it_img_beam'+nm_ind+ktkt[kk]+'/*.sav',count=count) & $
  image_sav='./imagetest/'+data_namet+'/it_clean_img'+nm_ind+ktkt[kk]+'/' & data_sav='./sav/'+data_namet+'/it_clean_img'+nm_ind+ktkt[kk]+'/'
...
;    img=abs(img)
    ...
    TIMES=2;set the times for cleaning
    ...
    while iteration lt 10 do begin;;200
      img_sl=make_array(pix,pix)
      img_sl[region_sl_index]=img[region_sl_index]
