;.r plt_data_automode_new_for_old_data.pro
...
; 
; Notes :
; This procedure is for the old vision of MUSER raw data ....
; 
; History :
; Wei Wang, 
...
if sys eq 0 then FILEt=FILE_SEARCH('I:\muserdata_20141217\MUSERra4\','*',count=count)
if sys eq 1 then FILEt=FILE_SEARCH('/Volumes/Seagate_cxy/muserdata_20141217/MUSERra4/','*',count=count)

nfile=count
nfile=1

FOR ii=1,40 DO BEGIN 
...
;-------------------------adding------------------------- 
if sys eq 0 then FILE=FILE_SEARCH('I:\muserdata_20141217\MUSERra4\',string(file_temp))
if sys eq 1 then FILE=FILE_SEARCH('/Volumes/Seagate_cxy/muserdata_20141217/MUSERra4/',string(file_temp))

FILE=FILE(SORT(FILE))
N_FILE=N_ELEMENTS(FILE)

N_FILE=1 ;for N_FILE min per .sav file.

NUM=2400L ;num/min, no more than 2400L, 25ms.
...
cor_data=lonarr(N_FILE*NUM,16,44,44,2)

pwr=lonarr(N_FILE*NUM,16,44)

INT_TIME=1L ;for integration time per min.
...
if num ge 100 and num lt 1000 then data_name_sav=data_name+replicate('num0',8)+replicate(strmid(NUM,9,3),8)
if num ge 1000 then data_name_sav=data_name+replicate('num',8)+replicate(strmid(NUM,8,4),8)
...
;-------------------------adding------------------------- 
freq_code_name=[0,85,170,255]

for aa=0,5 do begin ;for which frequency band.
file_mkdir,data_name_sav[aa]

OPENw,LUN2,'log_'+data_name[aa]+'.txt',/get_lun
AFF=1
AFF=0;which file for setting
...
while ((freq_code ne freq_code_name[fix(aa/2)]) or (freq_code ne freq_code1) ) do begin

  point_lun,1,99264LL+KK*320LL*100000LL*(60/NUM)+OFFSET0+OFFSET*bb
  READU,1,FREQ_CODE

  point_lun,1,99264LL+KK*320LL*100000LL*(60/NUM)+OFFSET0+OFFSET*(bb+1)
  READU,1,FREQ_CODE1

   ;print,freq_code
;    if FREQ_CODE eq 0 then begin
;     printF,LUN2,'0.4-0.8GHz'
;    ENDIF
;
;    if FREQ_CODE eq 85 then begin
;     printF,LUN2,'0.8-1.2GHz'
;    endif
;
;    if FREQ_CODE eq 170 then begin
;     printF,LUN2,'1.2-1.6GHz'
;    endif
;
;    if FREQ_CODE eq 255 then begin
;     printF,LUN2,'1.6-2.0GHz'
;    endif
...
endwhile
  ...
  if ((aa mod 2) ne 0) then bb=bb+1            ;if need R polarization, bb=bb+1, add one frame
   if (bb eq 9) then bb=1
   ;bb=(bb mod 8)
   point_lun,1,32+KK*100000LL*(60*320LL/NUM)+OFFSET0+OFFSET*(bb-1)                       ;read GPS time,100000L:DATA LENGTH OF EACH 3MS, 320LL:there are 320 sector in 1 second data.
   readu,1, gpst
...
;read data
   ...
   point_lun,1,2944lL+KK*100000LL*(60*320LL/NUM)+OFFSET0+OFFSET*INT*8+OFFSET*(bb-1)
...
FOR I=0,43 DO BEGIN
 FOR K=0,15 DO BEGIN
  FOR J=0,43 DO BEGIN
   FOR L=0,1 DO BEGIN
    COR_DATA[N_F*NUM+KK,K,I,J,L]=MEAN(COR_DATA_INT[I,J,K,L,*])
   ENDFOR
  ENDFOR
  PWR[N_F*NUM+KK,K,I]=MEAN(PWR_INT[I,K,*])
 ENDFOR
ENDFOR
;===============================
...
;SAVE,PWR,GPS_TIME,FILENAME='COR_data_'+data_name[aa]+'.sav'
